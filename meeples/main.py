...
def draw_toolbar(screen, selected_tool, speed_multiplier, meeple_count, cycle_info):
    ...
    # Info Text
    y_offset = icon_y + 15 # Start below icons
    # Define a consistent starting X position, inset from the toolbar edge
    text_margin = 5 # Small margin from the left edge of the toolbar
    text_x = SIMULATION_WIDTH + text_margin

    # Render and blit each line of text at text_x
    speed_text_surface = info_font.render(f"Speed: {speed_multiplier:.1f}x", True, INFO_TEXT_COLOR)
    screen.blit(speed_text_surface, (text_x, y_offset))
    y_offset += 20 # Increase y_offset for next line (adjust spacing if needed)

    meeple_text_surface = info_font.render(f"Meeples: {meeple_count}", True, INFO_TEXT_COLOR)
    screen.blit(meeple_text_surface, (text_x, y_offset))
    y_offset += 20

    # Check if cycle_info text is too wide (optional but good practice)
    cycle_text_surface = info_font.render(f"Cycle: {cycle_info}", True, INFO_TEXT_COLOR)
    available_width = TOOLBAR_WIDTH - (text_margin * 2) # Max width available
    if cycle_text_surface.get_width() > available_width:
        # If too wide, maybe shorten the text or use an even smaller font
        # For now, just blit it, but it might overflow
        pass # Consider adding handling for very long text later
    screen.blit(cycle_text_surface, (text_x, y_offset))
    y_offset += 20

    controls_text1_surface = info_font.render("'+'/'-': Speed", True, INFO_TEXT_COLOR)
    screen.blit(controls_text1_surface, (text_x, y_offset))
    # Optional: Add more controls text below if needed
    # y_offset += 20
    # controls_text2_surface = info_font.render("...", True, INFO_TEXT_COLOR)
    # screen.blit(controls_text2_surface, (text_x, y_offset))
